...
namespace PoeStashSearch.Api.Http {
	public interface IResourcesApiClient {
		Task<ICollection<League>> GetAllLeaguesAsync(CancellationToken stoppingToken = default);

		Task<ICollection<String>> GetAllRealmsAsync(CancellationToken stoppingToken = default);
	}
}
